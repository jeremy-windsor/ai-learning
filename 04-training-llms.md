# Module 04 — Training LLMs

**Mastery: 25% → 33%** · Prerequisites: Modules 02–03

**Goal of this module:** Understand what it actually takes — in data, compute, money, and time — to *create* a foundation model. By the end you'll be able to explain the difference between pretraining, fine-tuning, and inference, and why pretraining is reserved for a handful of labs on Earth.

---

## Chapter 1 — The Pretraining Objective

### §4.1 — Next-Token Prediction Is the Whole Game
...
### §4.4 — One Forward Pass, N Loss Terms
...
### §4.7 — Data Quality > Data Quantity (Eventually)
...
### §4.12 — FLOPs and the 6N Rule of Thumb
A useful approximation: training a dense transformer requires about `6 × N × D` FLOPs, where N = parameters and D = training tokens. For a 70B model on 15T tokens: ~6.3 × 10²⁴ FLOPs. An H100 peaks at ~1 PFLOP/s BF16 *with sparsity* (~0.5 PFLOP/s dense, which is what training actually gets), so a perfectly efficient run would need roughly **400 GPU-years**. Real life is worse: Meta reported ~6.4M H100-hours (~730 GPU-years) for Llama 3 70B — real-world utilization (MFU) is typically 30–45% of peak due to communication, restarts, and memory stalls.

### §4.13 — GPU-Months and the Money
...
### §4.14 — Why You Can't Pretrain at Home
...
### §4.15 — Mixed Precision Training
Train weights in BF16 (16-bit) for speed, but keep an FP32 master copy for stability. Combined with **loss scaling**, this 2× speedup is universal in modern training. (Quantization for *inference* is a separate, more aggressive game — Module 07.)

---

## Chapter 4 — Scaling Laws and Chinchilla

### §4.16 — The Scaling Hypothesis
...
### §4.17 — Chinchilla (2022) — The Course Correction
...
### §4.19 — Compute-Optimal vs Inference-Optimal
...
### §4.20 — The Bitter Lesson, Quantified
...
## Chapter 5 — Training Infrastructure (preview of Module 10)

### §4.21 — Why Training Is Different from Inference
...
### §4.22 — Data Parallelism (DDP)
Easiest scaling: copy the entire model to N GPUs, give each a different batch slice, average the gradients. Works until the model no longer fits on one GPU. Module 10 covers this in depth.

### §4.23 — Model/Tensor/Pipeline Parallelism
...
### §4.24 — Checkpointing and Restarts
...
### §4.25 — The Loss Curve and What Goes Wrong
...
### §4.26 — Base Models vs Instruct Models
Pretraining produces a **base model** — a raw next-token predictor. It doesn't follow instructions; ask it a question and it might generate more questions. **Instruction tuning** (Module 05) turns a base model into a helpful assistant. Both are released for Llama, Qwen, Mistral, etc.

### §4.27 — Why Open Weights Matter
...
### §4.29 — The Compute Concentration
...
### §4.30 — Checkpoint — Explain it Back
**Quiz yourself:**
