# Module 08 — Inference Engines

**Mastery: 58% → 67%** · Prerequisites: Modules 06–07

**Goal of this module:** Understand the software that actually *runs* a trained model — from `ollama run` on your laptop to `vllm serve` on a 8×H100 box. By the end you'll be able to recommend the right engine for any deployment scenario.

---

## Chapter 1 — Why Inference Needs Its Own Software

### §8.1 — Training Frameworks Aren't Inference Engines
...
### §8.2 — The Two Phases of Inference: Prefill vs Decode
...
Different optimizations apply to each. Modern engines treat them separately (see "disaggregated serving," §8.24).

### §8.3 — The KV Cache (full deep dive in Module 09)
...
### §8.5 — The Engine Landscape (the players)
...
## Chapter 2 — Local Inference: llama.cpp and Ollama

### §8.6 — llama.cpp: The Project That Started It All
...
### §8.8 — Ollama: The Friendly Wrapper
...
### §8.9 — How Ollama Picks Quantization
...
### §8.10 — When Ollama/llama.cpp Is the Right Choice
...
### §8.11 — When Ollama/llama.cpp Is **Not** the Right Choice
...
## Chapter 3 — Production Inference: vLLM

### §8.12 — vLLM: The Open-Source Workhorse
Born at UC Berkeley, 2023. Now the de-facto open-source standard for production LLM serving. Used by everyone from indie startups to LMSYS Arena. Core innovation: **PagedAttention** (§8.13).
...
### §8.16 — Tensor Parallelism in vLLM
...
### §8.19 — vLLM Architecture in 30 Seconds
...
## Chapter 4 — TensorRT-LLM, TGI, SGLang, Others
...
### §8.21 — TGI (Hugging Face Text Generation Inference)
...
### §8.23 — DeepSpeed-MII, MLC-LLM, ExLlamaV2
...
### §8.24 — Disaggregated Serving (the cutting edge, 2024–2025)
...
### §8.25 — Choosing an Engine: The Decision Tree
...
### §8.26 — How OpenAI/Anthropic Probably Do It
...
### §8.28 — Speculative Decoding (preview of Module 09)
...
### §8.29 — Tool Calling / JSON Mode
Modern engines support **constrained generation** — forcing the model's output to match a JSON schema, regex, or grammar. Done via masking the next-token logits at sampling time. Critical for agentic workloads (Module 12). Available in vLLM (`outlines`, `xgrammar`), SGLang, llama.cpp (`grammars`), TGI.
...
### §8.30 — Checkpoint — Explain it Back
**Quiz yourself:**
...
You can now answer "How does Ollama work?" *and* "How does OpenAI serve at scale?" in real depth. Next module: the optimization tricks behind every fast engine.
